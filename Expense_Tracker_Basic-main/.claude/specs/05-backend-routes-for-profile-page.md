...
- `database/queries.py` — pure query helpers with no Flask imports; one function per data concern:
  ...
  - `get_summary_stats(user_id)`
    ...
    - If the user has no expenses, returns `{"total_spent": 0, "transaction_count": 0, "top_category": "—"}`
  ...
  - `get_category_breakdown(user_id)`
    ...
    - `pct` values must sum to 100; the largest category absorbs any rounding remainder
